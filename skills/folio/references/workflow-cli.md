# Folio amendment workflow — CLI

When the `folio` CLI is installed, the ritual is:

```bash
folio draft <topic>     # opens an amendment worktree on amend/<topic>
...
- **GitHub mode** — `proof` pushes the amendment branch and opens or updates a draft PR. `publish` squash-merges into the default branch.
...
```bash
folio drop              # deletes the amendment branch and worktree
```
