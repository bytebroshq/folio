...
Use `-m <message>` when the change needs an intentional, polished public summary. The message becomes the commit message; with PR strategy, its first line becomes the PR title and the full message becomes the PR body. On a subsequent proof, supplying `-m` intentionally replaces the existing PR title and body. Omit `-m` for routine follow-up proofs: Folio uses `amend: <topic>` for the commit and preserves existing PR metadata. When invoking Folio through a shell, pass messages containing Markdown code spans or shell substitutions as one shell-safe argument.
