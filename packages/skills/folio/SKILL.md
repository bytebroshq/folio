...
When proofing, use `-m <message>` sparingly. Use it for a polished change
summary that should become the commit message and, with PR strategy, the PR
title/body. It intentionally replaces existing PR metadata on a subsequent
proof. Omit `-m` for routine follow-up proofs so manually edited PR title/body
